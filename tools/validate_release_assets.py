"""Static release-asset validation for the MoLFormer-XL molecular-property DIMER pipeline.
...
**One fleet rule is relaxed here, deliberately and narrowly.** The fleet validator forbids
`trust_remote_code=True` anywhere in a tutorial. This checkpoint cannot be loaded without it -- its
`config.json` has no native `model_type` and its linear-attention architecture is implemented only
in the upstream Python -- so the ban becomes a *location* rule enforced by
`validate_remote_code_boundary`: enabled only inside the carried module cell, forbidden everywhere
else, with the two executed Python files listed in the snapshot manifest and verified before import.
See MODEL_CARD.md (DIMER deployment notes) for the open gate this leaves.
...
REPO_NAME = "molformer-chemistry-pipeline"
NOTEBOOK_NAME = "molformer_chemistry_colab.ipynb"
EXPECTED_PROFILE = "E2E"
EXPECTED_MODEL_ID = "ibm-research/MoLFormer-XL-both-10pct"
...
BYOD_GATES = ("USE_BYOD",)
# The Python files the loader executes; they must be manifest entries (MODEL_ASSET_SPEC RC3).
REMOTE_CODE_FILES = ("configuration_molformer.py", "modeling_molformer.py")
EXPECTED_OUTPUTS = (
    "outputs/molformer_chemistry_sample_dataset.csv",
    "outputs/molformer_chemistry_embeddings.csv",
    "outputs/molformer_chemistry_evaluation_report.json",
    "outputs/molformer_chemistry_predictions.csv",
    "outputs/molformer_chemistry_adapter",
    "outputs/molformer_chemistry_result.json",
)
CODE_MARKERS = (
    ...
    # Stage 5: tokenization, ceilings and rejection probes
    "pipe.token_count(smiles)",
    "'max_tokens': MAX_TOKENS",
    "input_manifest = validate_inputs(",
    "token_counter=pipe.token_count",
    # Stage 6: embeddings, and the determinism check this checkpoint needs
    "embedding_result = pipe.embed([r['smiles'] for r in embed_records], names=[r['id'] for r in embed_records])",
    "'deterministic_eval': DETERMINISTIC_EVAL",
    "'same_vector_on_a_second_call': repeat == vectors[0]",
    # Stage 7: baselines fitted on train only
    ...
    # Stage 8: bounded fine-tuning
    "adapt_result = pipe.adapt(",
    "trainable_layers=TRAINABLE_LAYERS",
    "learning_rate=LEARNING_RATE",
    # Stage 9: held-out evaluation
    ...
    # Stage 10: inference, artifact export with serving state, reload parity
    ...
    "'serving_state_tensors': len(artifact_manifest['serving_state_tensors'])",
    "reloaded_pipe = MolformerPipeline.from_artifact(artifact_dir, weights_dir=WEIGHTS_DIR)",
    "assert max_score_diff < 1e-5",
    # Stage 11: provenance, including the remote-code disclosure
...
MARKDOWN_MARKERS = (
    ...
    "representations",
    "syntactic",
    "scaffold or cluster split",
    "trust_remote_code",
)
# Direct-library use that must stay inside the carried module cells (G2). `trust_remote_code` is
# handled separately by validate_remote_code_boundary, because here it is unavoidable.
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    1_519: "upstream .gitattributes of ibm-research/MoLFormer-XL-both-10pct at 361063d",
    3_545: "upstream convert_molformer_original_checkpoint_to_pytorch.py at 361063d (not staged)",
    241_085: "upstream pipeline.jpeg card illustration at 361063d (not staged)",
}
...
def validate_remote_code_boundary() -> None:
    """`trust_remote_code=True` is allowed ONLY inside the carried module cell, and only because this
    checkpoint cannot be loaded without it (MODEL_ASSET_SPEC RC2: architectural requirement, not a
    workaround for a loading error -- `AutoModel.from_pretrained(..., trust_remote_code=False)`
    refuses it). Everywhere else in the notebook it stays forbidden, the two Python files the loader
    executes must be manifest entries, and the module must verify the snapshot before importing them.
    """
    build = _load_tool("build_notebook")
    template = _load_tool("notebook_template").TEMPLATE
    notebook = json.loads(_read(ROOT / "tutorials" / NOTEBOOK_NAME))
    embedded_indices = {
        index
    ...
    for index, cell in enumerate(notebook["cells"]):
        if cell.get("cell_type") != "code" or index in embedded_indices:
            continue
        _check(
            not pattern.search(_cell_source(cell)),
            f"trust_remote_code=True appears outside the carried module cell (cell {index})",
        )
    module = _read(ROOT / "src" / PACKAGE / "pipeline.py")
    _check(pattern.search(module) is not None, "the pipeline module must be where remote code is enabled")
    for name in REMOTE_CODE_FILES:
        _check(
            f'"{name}"' in module,
            f"pipeline.py must name the remote-code file {name} so its digest is verified",
        )
    manifest = json.loads(_read(ROOT / "weights" / template["weights_key"] / "dimer-base-manifest.json"))
    listed = {entry["path"] for entry in manifest["files"]}
    missing = [name for name in REMOTE_CODE_FILES if name not in listed]
    _check(not missing, f"the snapshot manifest must list the executed Python files, missing {missing}")
    verify_at = module.index("verify_snapshot(root)")
    import_at = module.index("from transformers import AutoModel, AutoTokenizer")
    _check(verify_at < import_at, "verify_snapshot must run before the model libraries are imported")
    del build
...
def validate_all() -> list[str]:
    ...
    validate_notebooks()
    validate_remote_code_boundary()
    return ["model-card", "identity-consistency", "weight-facts", "release-status", "notebook+parity", "remote-code-boundary"]
